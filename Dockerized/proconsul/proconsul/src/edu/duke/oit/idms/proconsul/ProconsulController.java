...
public class ProconsulController {
	...
	public ModelAndView handleReconnectPost(@ModelAttribute ReconnectRequest rr,HttpServletRequest request) {
		...
		LOG.info("Reconnect request received from " + au.getUid() + "using client address " + request.getRemoteAddr());
		...
		LOG.info("Verified canUseApp for recon request from " + au.getUid() + " using client address " + request.getRemoteAddr());
		
		// CSRF protections
		if (! ProconsulUtils.checkOrigin(request)) {
			LOG.info("CSRF violation from client address " + request.getRemoteAddr());
			ModelAndView errModel = new ModelAndView("/authzError");
		...
		if (token != null) {
			...
			if (! token.equals(validToken)) {
				LOG.info("CSRF token violation from client address " + request.getRemoteAddr());
				ModelAndView errModel = new ModelAndView("/authzError");
				errModel.addObject("message","CSRF token violation -- your browser is not authorized for this operation.");
				return errModel;
			}
		} else {
			LOG.info("CSRF token violation -- no token present in request from client address " + request.getRemoteAddr());
			ModelAndView errModel = new ModelAndView("/authzError");
		...
		if (false) {  // temporarily removed
			...
			LOG.info("Reconnect starting new container for " + au.getUid() + " using client address " + request.getRemoteAddr() + " due to session status " + reconnectSession.getStatus().toString());
		}
		...
		LOG.info("Wrote updated reconnect session to database for " + au.getUid() + " using client address " + request.getRemoteAddr() + " with target user " + reconnectSession.getTargetUser().getsAMAccountName() + " and target sever " + reconnectSession.getFqdn());
		...
		LOG.info("Created new container for " + au.getUid() + " using client address " + request.getRemoteAddr() + " with target user " + reconnectSession.getTargetUser().getsAMAccountName() + " and target server " + reconnectSession.getFqdn());
		if ((request.getParameter("rresolution") != null && request.getParameter("rresolution").equals("large")) || (request.getParameter("rdaresolution") != null && request.getParameter("rdaresolution").equals("large")))
			...
			container.start();
		LOG.info("Started container for " + au.getUid() + " using client address " + request.getRemoteAddr() + " with target user " + reconnectSession.getTargetUser().getsAMAccountName() + " and target server " + reconnectSession.getFqdn());
	...
	public ModelAndView handleDomainAdminPost(@ModelAttribute DomainAdminRequest domreq,HttpServletRequest request) {
		...
		AuthUser au = new AuthUser(request);
		LOG.info("Received domain admin login request from " + au.getUid() + "using client address " + request.getRemoteAddr());
		...
		LOG.info("Verified canUseApp for " + au.getUid() + " using client address " + request.getRemoteAddr());
		
		if (! ProconsulUtils.canUseDA(au)) {
			ModelAndView errModel = new ModelAndView("/authzError");
			errModel.addObject("message","You are not a domain admin -- this will be reported");
			LOG.info("User " + au.getUid() + " using client address " + request.getRemoteAddr() + " attempt to use Proconsul for domain admin access failed to to authz error");
			return errModel;
		}
		
		// CSRF protections
		if (! ProconsulUtils.checkOrigin(request)) {
			LOG.info("CSRF violation for " + au.getUid() + " using client address " + request.getRemoteAddr());
			ModelAndView errModel = new ModelAndView("/authzError");
			errModel.addObject("message","CSRF origin violation -- your browser is not authorized for this operation.");
			return errModel;
		}
		...
		if (token != null) {
			String validToken = ProconsulUtils.getSessionCSRFToken(request);
			if (! token.equals(validToken)) {
				LOG.info("CSRF token violation for " + au.getUid() + "using client address " + request.getRemoteAddr());
				ModelAndView errModel = new ModelAndView("/authzError");
				errModel.addObject("message","CSRF token violation -- your browser is not authorized for this operation");
				return errModel;
			}
		} else {
			LOG.info("CSRF token violation -- no token presented for " + au.getUid() + " using client address " + request.getRemoteAddr());
			ModelAndView errModel = new ModelAndView("/authzError");
		...
		if (! dafqdns.contains(targetFQDN)) {
			...
			LOG.info("User " + au.getUid() + " using client address " + request.getRemoteAddr() + " attempt to make DA connection to " + targetFQDN + " denied because host is not authorized for DA access");
			return errModel;
		}
		...
		LOG.info("Created new vnc password for " + au.getUid() + " using client address " + request.getRemoteAddr());
		// ADUser targetUser = ProconsulUtils.createRandomizedADUser();
		ADUser targetUser = ProconsulUtils.createRandomizedADUser(au);
		LOG.info("Created new target AD user " + targetUser.getsAMAccountName() + " for " + au.getUid() + " using client address " + request.getRemoteAddr());
		ProconsulSession session = new ProconsulSession(au,targetFQDN,vncPassword,targetUser,displayName,"domain");
		LOG.info("Created new session (" + au.getUid() + "," + targetFQDN + "," + targetUser.getsAMAccountName() + "," + displayName + ",domain)");
		...
		if (! ProconsulUtils.makeUserAuthorizedDA(targetUser)) {
			...
			LOG.info("Failed to update proconsul database with new AD admin " + targetUser.getsAMAccountName() + " -- user " + au.getUid() + " session failed");
			return errModel;
		}
		...
		if (updated == null) {
			...
			ModelAndView errModel = new ModelAndView("/authzError");
			errModel.addObject("message","Unable to update authorization to allow DA access - try again later");
			LOG.info("Failed to update protocol database with new AD admin " + targetUser.getsAMAccountName() + " -- user " + au.getUid() + " session failed");
			return errModel;
		}
		...
		LOG.info("Created new container for " + au.getUid() + " using client address " + request.getRemoteAddr() + " with target user " + session.getTargetUser().getsAMAccountName() + " and target server " + session.getFqdn());
	...
	public ModelAndView handleDelegatedAdminPost(@ModelAttribute DelegatedAdminRequest deladmin, HttpServletRequest request) {
		...
		AuthUser au = new AuthUser(request);
		LOG.info("Received delegated admin login request from " + au.getUid() + "using client address " + request.getRemoteAddr());
		
		if (! ProconsulUtils.canUseApp(au)) {
			ModelAndView errModel = new ModelAndView("/authzError");
			errModel.addObject("message","You (" + au.getUid()+") are not authorized to use this application");
			LOG.info("User " + au.getUid() + " using client address " + request.getRemoteAddr() + " attempt to use Proconsul for delegated admin failed due to authz error");
			return errModel;
		}
		LOG.info("Verified canUseApp for " +au.getUid()+ " using client address " + request.getRemoteAddr());
		...
		if (! groupOK) {
			...
			LOG.info("User " + au.getUid() + " using client address " + request.getRemoteAddr() + " failed attempt to claim role group " + roleGroup);
			return errModel;
		}
		...
		if (token != null) {
			...
			LOG.info("CSRF token violation for " + au.getUid() + " using client address " + request.getRemoteAddr() + " -- no token presented");
			ModelAndView errModel = new ModelAndView("/authzError");
		...
		LOG.info("Created new session (" + au.getUid() + "," + targetFQDN + "," + targetUser.getsAMAccountName() + "," + displayName + ",delegated," + orgUnit + "," + roleGroup + ")");
		...
		if (updated == null) {
			...
			LOG.info("Failed to add target host group for " + au.getUid() + " using client address " + request.getRemoteAddr() + " failing login");
			return errModel;
		}
		...
		if (updated2 == null) {
			...
			LOG.info("Failed to add delegated admin group for " + au.getUid() + " using client address " + request.getRemoteAddr() + " failing login");
			return errModel;
		}
		...
		if (updated3 == null) {
			...
			LOG.info("Failed to add requested delegate group for " + au.getUid() + " using client address " + request.getRemoteAddr() + " -- " + roleGroup + " -- failing login");
			return errModel;
		}
		...
		DockerContainer container = new DockerContainer(session);
		LOG.info("Created new container for " + au.getUid() + " using client address " + request.getRemoteAddr());
	...
	public ModelAndView handleStaticSessionPost(@ModelAttribute StaticUserSessionRequest staticsession, HttpServletRequest request) {
		AuthUser au = new AuthUser(request);
		LOG.info("authuser uid is " + au.getUid() + " using client address " + request.getRemoteAddr());
		if (! ProconsulUtils.canUseApp(au)) {
		...
		LOG.info("Created new session (" + au.getUid() + "," + targetFQDN + "," + targetUser.getsAMAccountName() + "," + displayName + ",static)");
		...
		LOG.info("Wrote new session to db for " + au.getUid() + " using client address " + request.getRemoteAddr());
		...
		String redirectUrl = session.getRedirectUrl();
		LOG.info("Sending redirect URL for " + au.getUid() + " using client address " + request.getRemoteAddr());
		return new ModelAndView("redirect:" + redirectUrl);
	...
	public ModelAndView handleUserSessionPost(@ModelAttribute UserSessionRequest usersession, HttpServletRequest request) {
		...
		LOG.info("Verified host authorization for " + au.getUid() + " using client address " + request.getRemoteAddr() + " accessing " + usersession.getTargetFQDN());
		...
		//ADUser targetUser = ProconsulUtils.createRandomizedADUser();
		ADUser targetUser = ProconsulUtils.createRandomizedADUser(au);
		LOG.info("Created new AD targetuser " + targetUser.getsAMAccountName() + " for " + au.getUid() + " using client address " + request.getRemoteAddr());
		...
		LOG.info("Created new session (" + au.getUid() + "," + targetFQDN + "," + targetUser.getsAMAccountName() + "," + displayName + ",user)");
		...
		LOG.info("Added user " + targetUser + " to default group " + defgroupdn);
		...
		for (String dn : ProconsulUtils.groupDnsForEppn(au.getUid())) {
			LOG.info("Adding group " + dn + " to AD user for " + au.getUid() + " using client address " + request.getRemoteAddr());
	...
	public ModelAndView handleRootRequest(HttpServletRequest request) {
		...
		if (csrfToken == null) {
			LOG.info("CSRF token creation failure from " + request.getRemoteAddr());;
			ModelAndView errModel = new ModelAndView("/authzError");
		...
		LOG.info("Root GET request received from " + request.getRemoteAddr());
		...
		if (! ProconsulUtils.canUseApp(testUser)) {
			...
			LOG.info("User " + testUser.getUid() + " using client address " + request.getRemoteAddr() + " is not authorized to use Proconsul");
			return errModel;
		}
